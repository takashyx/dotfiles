#!/usr/bin/env bash
# =====================================================================
# dotfiles セットアップエントリポイント (WSL)
#   各モジュールフォルダ直下の setup.wsl.sh を順に実行する。
#   WSL 対応の設定を追加するときは、フォルダに setup.wsl.sh を置くだけでよい
#   (置かないモジュールは単にスキップされる)。
#   モジュールが失敗しても残りは続行し、最後に結果をサマリー表示する。
# 実行例 (WSL 内のシェルから): bash setup.wsl.sh
# =====================================================================
set -uo pipefail
...
for script in "$REPO_ROOT"/*/setup.wsl.sh; do
  [ -f "$script" ] || continue
